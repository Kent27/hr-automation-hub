...
- For claim tests, pass `amount_override` (or inject fake OCR/Ollama services) to avoid runtime OCR/model dependencies.
- For holiday/email automations, inject fake services instead of real API clients.
- For holiday sync tests, prefer fake OpenAI/Ollama services and override PDF/image render helpers to keep tests offline and deterministic.
...
- Networked integrations (Ollama/Gmail) should be avoided in test paths unless explicitly integration-testing.
...
| `test_holiday_sync_service.py` | Holiday extractor tests for OpenAI vision-first flow, Ollama fallback, and failure alert path. |
| `test_invoice_parser.py` | Hybrid invoice parser tests (rules, LLM fallback, and failure alert path). |
